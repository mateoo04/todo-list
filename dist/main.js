...
var editTaskDialog = document.querySelector(".edit-task-dialog");
function setUpUserInterface() {
  var NEW_TASK = "new task created",
  ...
  var newTaskDialog = document.querySelector(".new-task-dialog");
  ...
  var newTaskForm = document.querySelector(".new-task-dialog form");
  newTaskForm.addEventListener("submit", function (event) {
    event.preventDefault();
    var title = document.querySelector("#title-input").value;
    var dueDate = document.querySelector("#due-date-input").value;
    var priority = document.querySelector("#priority-input").value;
  ...
  var newListDialog = document.querySelector(".new-list-dialog");
  document.querySelector(".add-list-button").addEventListener("click", function () {
    newListDialog.showModal();
  });
  var newListForm = document.querySelector(".new-list-dialog form");
  ...
  var closeDialogButtons = document.querySelectorAll(".close-dialog-button");
  closeDialogButtons.forEach(function (element) {
...
function updateTaskList(list) {
  taskList.innerHTML = "";
  list.tasks.forEach(function (value) {
...
function generateTaskItemElement(task) {
  ...
  var EDIT_TASK = "edit task button clicked";
  var detailsButton = document.createElement("button");
  detailsButton.classList.add("details-button");
  detailsButton.style.backgroundImage = "url(".concat(_info_icon_svg__WEBPACK_IMPORTED_MODULE_1__, ")");
  detailsButton.addEventListener("click", function () {
    pubsub_js__WEBPACK_IMPORTED_MODULE_0___default().publish(EDIT_TASK, task.id);
  });
  var deleteTaskButton = document.createElement("button");
  deleteTaskButton.classList.add("delete-task-button");
  deleteTaskButton.style.backgroundImage = "url(".concat(_trash_icon_svg__WEBPACK_IMPORTED_MODULE_2__, ")");
  ...
  var noetDetail = document.createElement("p");
  noetDetail.classList.add("note-priority");
  noetDetail.textContent = task.note;
  var onHoverDetails = document.createElement("div");
  onHoverDetails.classList.add("on-hover-details");
  var priorityDetail = document.createElement("p");
...
function updateListsList(taskLists) {
  var ACTIVE_LIST_CHANGE = "active list changed";
  var listsList = document.querySelector(".lists-list");
  listsList.innerHTML = "";
...
function showAllTasks(lists) {
  ...
  for (var i = 0; i < lists.length; i++) {
    lists[i].tasks.forEach(function (value) {
      taskList.append(generateTaskItemElement(value));
    });
  }
}
function updateActiveListHeader(title) {
  var activeListHeader = document.querySelector(".active-list-header");
  activeListHeader.textContent = title;
}
function requestTaskChanges(changeObject) {
  var TASK_MODIFICATION = "task modified";
...
function showEditTaskDialog(id, setTitle, setDueDate, setPriority, setNote) {
  var editTaskForm = document.querySelector(".edit-task-dialog form");
  var titleInput = document.querySelector("#edit-title-input");
  var dueDateInput = document.querySelector("#edit-due-date-input");
  var priorityInput = document.querySelector("#edit-priority-input");
  var noteInput = document.querySelector("#edit-note-input");
  titleInput.value = "".concat(setTitle);
  dueDateInput.value = setDueDate;
  priorityInput.value = setPriority.toLowerCase();
  noteInput.value = setNote;
  var _submitEditFormHandler = function submitEditFormHandler(event) {
    event.preventDefault();
    requestTaskChanges({
      id: id,
    ...
    editTaskForm.reset();
    editTaskForm.removeEventListener("submit", _submitEditFormHandler);
  };
  editTaskDialog.showModal();
  editTaskForm.addEventListener("submit", _submitEditFormHandler);
  document.querySelector(".close-edit-dialog-button").addEventListener("click", function () {
    editTaskDialog.close();
    editTaskForm.removeEventListener("submit", _submitEditFormHandler);
  });
}
function requestTaskDeletion(id) {
  var confirmDeletionDialog = document.querySelector(".confirm-deletion-dialog");
  var deleteButton = document.querySelector(".delete-task");
  var _deleteTask = function deleteTask() {
    var DELETE_TASK = "delete task";
    pubsub_js__WEBPACK_IMPORTED_MODULE_0___default().publish(DELETE_TASK, id);
    confirmDeletionDialog.close();
    deleteButton.removeEventListener("click", _deleteTask);
  };

  //confirming if user wants task to be deleted
  confirmDeletionDialog.showModal();
  deleteButton.addEventListener("click", _deleteTask);
  document.querySelector(".close-deletion-dialog-button").addEventListener("click", function () {
    confirmDeletionDialog.close();
    deleteButton.removeEventListener("click", _deleteTask);
  });
}
function populateListSelect(lists, activeListTitle) {
  var select = document.querySelector("#list-select");
  select.innerHTML = "";
  lists.forEach(function (taskList) {
...
var NEW_TASK = "new task created";
var NEW_LIST = "new list created";
var ACTIVE_LIST_CHANGE = "active list changed";
var SHOW_ALL_TASKS = "show all tasks";
var TASK_MODIFICATION = "task modified";
var EDIT_TASK = "edit task button clicked";
var DELETE_TASK = "delete task";
var POPULATE_LIST_SELECT_REQUEST = "populate list select request";
var TaskList = /*#__PURE__*/function () {
  function TaskList(title) {
    var isRetrieving = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
  ...
  return _createClass(TaskList, [{
    ...
    value: function findByTitle(title) {
      return TaskList.allLists.find(function (list) {
    ...
    key: "resetActiveList",
    value: function resetActiveList() {
      TaskList.activeList = -1;
    }
  }, {
    key: "getTaskById",
    value: function getTaskById(taskId) {
      var _iterator = _createForOfIteratorHelper(TaskList.allLists),
        _step;
      try {
        for (_iterator.s(); !(_step = _iterator.n()).done;) {
          var list = _step.value;
          var _iterator2 = _createForOfIteratorHelper(list.tasks),
            _step2;
          try {
    ...
    key: "getListOfTaskByTaskId",
    value: function getListOfTaskByTaskId(taskId) {
      var _iterator3 = _createForOfIteratorHelper(TaskList.allLists),
        _step3;
      try {
        for (_iterator3.s(); !(_step3 = _iterator3.n()).done;) {
          var list = _step3.value;
          var _iterator4 = _createForOfIteratorHelper(list.tasks),
            _step4;
          try {
    ...
    value: function deleteTask(taskId) {
      var _iterator5 = _createForOfIteratorHelper(TaskList.allLists),
        _step5;
      try {
        for (_iterator5.s(); !(_step5 = _iterator5.n()).done;) {
          var list = _step5.value;
          var _iterator6 = _createForOfIteratorHelper(list.tasks),
            _step6;
          try {
...
_defineProperty(TaskList, "activeList", -1);
_defineProperty(TaskList, "allLists", []);
var Task = /*#__PURE__*/_createClass(function Task(title, listTitle) {
  var nonFormattedDueDate = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : "";
  ...
  var isRetrieving = arguments.length > 6 && arguments[6] !== undefined ? arguments[6] : false;
...
pubsub_js__WEBPACK_IMPORTED_MODULE_2___default().subscribe(NEW_TASK, function (msg, task) {
  if (validateTitle(task.title)) {
    new Task(task.title, task.list, task.dueDate, task.priority, task.note);
    requestUpdateTaskListInterface();
  }
});
pubsub_js__WEBPACK_IMPORTED_MODULE_2___default().subscribe(NEW_LIST, function (msg, newListTitle) {
...
pubsub_js__WEBPACK_IMPORTED_MODULE_2___default().subscribe(ACTIVE_LIST_CHANGE, function (msg, list) {
  list.setAsActive();
  requestListsListUpdate();
});
pubsub_js__WEBPACK_IMPORTED_MODULE_2___default().subscribe(SHOW_ALL_TASKS, function () {
  requestUpdateTaskListInterface();
  TaskList.resetActiveList();
});
pubsub_js__WEBPACK_IMPORTED_MODULE_2___default().subscribe(TASK_MODIFICATION, function (msg, changeObject) {
  if ("title" in changeObject) {
    if (!validateTitle(changeObject.title)) return -1;
  }
  var taskForModification = TaskList.getTaskById(changeObject.id);
  for (var _i = 0, _Object$entries = Object.entries(changeObject); _i < _Object$entries.length; _i++) {
    ...
    if (key === "nonFormattedDueDate") {
      if (value === "") taskForModification.dueDate = "";else taskForModification.dueDate = getDate(value);
    }
  }
  requestUpdateTaskListInterface();
  saveDataToLocalStorage(TaskList.getListOfTaskByTaskId(changeObject.id));
});
pubsub_js__WEBPACK_IMPORTED_MODULE_2___default().subscribe(EDIT_TASK, function (msg, selectedTask) {
  var task = TaskList.getTaskById(selectedTask);
...
pubsub_js__WEBPACK_IMPORTED_MODULE_2___default().subscribe(DELETE_TASK, function (msg, id) {
  var list = TaskList.getListOfTaskByTaskId(id);
  TaskList.deleteTask(id);
  requestUpdateTaskListInterface();
  saveDataToLocalStorage(list);
});
pubsub_js__WEBPACK_IMPORTED_MODULE_2___default().subscribe(POPULATE_LIST_SELECT_REQUEST, function () {
...
function generateId() {
  return "".concat(TaskList.getActiveList().title.toLowerCase().replaceAll(" ", ""), "-") + Date.now().toString().slice(-8) + "-" + Math.floor(Math.random() * 10000);
}
...
function retrieveDataFromLocalStorage() {
  if (storageAvailable("localStorage")) {
    var _loop = function _loop() {
      ...
      var listOfTasks = JSON.parse(localStorage.getItem(key));
...
if (TaskList.findByTitle("My List") === undefined) {
  var defaultList = new TaskList("My List");
